/**
 * Invoices Router — /api/invoices (ESM)
 * Property-scoped. Line items, HTML generation, payment recording.
 */
...
router.get('/', async (req, res) => {
  try {
    ...
    if (req.propertyId) q = q.where('i.property_id', req.propertyId);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (req.propertyId && inv.property_id && inv.property_id !== req.propertyId)
  ...
  try {
    const [inv] = await db('invoices').where({ id: req.params.id })
      .update({ status: 'paid', payment_method,
        ...
        notes: notes || null, paid_at: new Date(), updated_at: new Date() })
      .returning('*');
...
router.post('/:id/line-items', atLeast('receptionist'), async (req, res) => {
  ...
  if (!description || !unit_price)
    return res.status(400).json({ error: 'description and unit_price required' });
  try {
    ...
    const [updated] = await db('invoices').where({ id: req.params.id })
      .update({ line_items: JSON.stringify(items), extras_total: extrasTotal,
        tax_amount: taxAmount, grand_total: subtotal + taxAmount, updated_at: new Date() })
      .returning('*');
    res.json(updated);
  } catch (e) {
