/**
 * Housekeeping Router — /api/housekeeping (ESM)
 * Full workflow: pending → assigned → in_progress → completed → inspected
 * Property-scoped on all queries.
 */

import express from 'express';
import db      from '../db.js';
import auth    from '../middleware/auth.js';
import { propertyScope, atLeast, roles, scopeQuery } from '../middleware/rbac.js';
...
router.get('/', async (req, res) => {
  try {
    ...
    if (req.propertyId) q = q.where('ht.property_id', req.propertyId);
...
router.get('/staff', async (req, res) => {
  try {
    let q = db('users').where({ role: 'housekeeping' })
  ...
  try {
    const task = await db('housekeeping_tasks as ht')
      ...
      .select('ht.*',
        ...
        'cb.name as created_by_name', 'ib.name as inspected_by_name')
      .where('ht.id', req.params.id).first();
...
router.patch('/:id/assign', atLeast('manager'), async (req, res) => {
  ...
  try {
    ...
    if (!task) return res.status(404).json({ error: 'Task not found' });
    await db('rooms').where({ id: task.room_id })
      .update({ assigned_hk_user: assigned_to, updated_at: new Date() });
    res.json(task);
  } catch (e) {
...
router.patch('/:id/start', roles('housekeeping','manager','hotel_admin','super_admin'), async (req, res) => {
  try {
    ...
    await db('rooms').where({ id: task.room_id }).update({
      status: 'maintenance', housekeeping_status: 'in_progress',
      hk_started_at: new Date(), updated_at: new Date(),
    });
    res.json(updated);
  } catch (e) {
...
router.patch('/:id/complete', roles('housekeeping','manager','hotel_admin','super_admin'), async (req, res) => {
  ...
  try {
    ...
    if (task.status !== 'in_progress')
      return res.status(400).json({ error: `Task must be in_progress (current: ${task.status})` });
    const [updated] = await db('housekeeping_tasks').where({ id: req.params.id })
      .update({
        status: 'completed', completion_notes: completion_notes || null,
        checklist: checklist ? JSON.stringify(checklist) : task.checklist,
        completed_at: new Date(), updated_at: new Date(),
      }).returning('*');
    await db('rooms').where({ id: task.room_id })
      .update({ hk_completed_at: new Date(), updated_at: new Date() });
    res.json(updated);
  } catch (e) {
...
router.patch('/:id/inspect', atLeast('manager'), async (req, res) => {
  try {
    ...
    const [updated] = await db('housekeeping_tasks').where({ id: req.params.id })
      .update({ status: 'inspected', inspected_at: new Date(),
        inspected_by: req.user.id, updated_at: new Date() }).returning('*');
    await db('rooms').where({ id: task.room_id }).update({
      status: 'available', housekeeping_status: 'clean',
      assigned_hk_user: null, hk_started_at: null, hk_completed_at: null,
      updated_at: new Date(),
    });
    res.json(updated);
  } catch (e) {
